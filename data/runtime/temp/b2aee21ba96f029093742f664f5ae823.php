...
<style>
    ...
    <link href="/themes/simpleboot3/public/assets/css/product-info.css?v=20260612.19" rel="stylesheet" type="text/css">
    <script type="text/javascript" src="/themes/simpleboot3/public/assets/js/product-info.js?v=20260612.19"></script>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <?php if(!(empty($recommended_list) || (($recommended_list instanceof \think\Collection || $recommended_list instanceof \think\Paginator ) && $recommended_list->isEmpty()))): ?>
            <div class="hot-products">
                ...
                <div class="swiper hot-products-swiper hot-products-list">
                    <div class="swiper-wrapper">
                        <?php if(is_array($recommended_list) || $recommended_list instanceof \think\Collection || $recommended_list instanceof \think\Paginator): if( count($recommended_list)==0 ) : echo "" ;else: foreach($recommended_list as $key=>$vo): ?>
                            <div class="swiper-slide product_item">
                                ...
                                <div class="product_item__footer">
                                    ...
                                    <a class="product_list_item_btn detail-btn" href="<?php echo cmf_url('portal/index/product_info',array('id'=>$vo['id'])); ?>">View Details<span class="btn-arrow"></span></a>
                                </div>
                            </div>
                        <?php endforeach; endif; else: echo "" ;endif; ?>
                    </div>
        ...
        <!-- INQUIRY FORM START -->
        <div class="home-collect" id="detail-inquiry-form">
            <div class="home-collect-container">
                ...
                <div class="home-collect-form-section">
                    <div class="home-collect-form">
                        ...
                        <div class="home-collect-submit" onclick="submitDetailInquiry()">Submit Request</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- EXPERT CONTACT START -->
        <?php 
            $expertWhatsapp = !empty($site_info['expert_whatsapp']) ? $site_info['expert_whatsapp'] : $site_info['whatsapp'];
            $expertCleanNumber = '';
            if (!empty($expertWhatsapp)) {
                $expertCleanNumber = preg_replace('/[^0-9]/', '', $expertWhatsapp);
            }
            $expertEmail = !empty($site_info['expert_email']) ? $site_info['expert_email'] : $site_info['email'];
         ?>
        <div class="expert-contact-block">
            <div class="expert-photo">
                <?php if(empty($site_info['expert_photo'])): ?>
                    <img src="/themes/simpleboot3/public/assets/images/expert-zora.png" alt="<?php echo (isset($site_info['expert_name']) && ($site_info['expert_name'] !== '')?$site_info['expert_name']:'Zora Zhao'); ?>" onerror="this.src='https://via.placeholder.com/200x250?text=Expert+Photo'">
                    <?php else: ?>
                    <img src="<?php echo cmf_get_image_url($site_info['expert_photo']); ?>" alt="<?php echo (isset($site_info['expert_name']) && ($site_info['expert_name'] !== '')?$site_info['expert_name']:'Zora Zhao'); ?>">
                <?php endif; ?>
            </div>
            <div class="expert-info">
                <div class="expert-name"><?php echo (isset($site_info['expert_name']) && ($site_info['expert_name'] !== '')?$site_info['expert_name']:'Zora Zhao'); ?></div>
                <div class="expert-title"><?php echo (isset($site_info['expert_title']) && ($site_info['expert_title'] !== '')?$site_info['expert_title']:'Expert in Overhead Crane/Gantry Crane/Jib Crane/Crane Parts Solutions'); ?></div>
                <div class="expert-desc">
                    <?php echo (isset($site_info['expert_desc']) && ($site_info['expert_desc'] !== '')?$site_info['expert_desc']:'With 10+ years of experience in the Crane Overseas Export Industry, helped 10,000+ customers with their pre-sales questions and concerns, if you have any related needs, please feel free to contact me!'); ?>
                </div>
                <div class="expert-contact-details">
                    <p>WhatsApp: <a href="https://api.whatsapp.com/send?phone=<?php echo $expertCleanNumber; ?>" target="_blank"><?php echo (isset($expertWhatsapp) && ($expertWhatsapp !== '')?$expertWhatsapp:'+86 189 3735 0200'); ?></a></p>
                    <p>Email: <a href="mailto:<?php echo (isset($expertEmail) && ($expertEmail !== '')?$expertEmail:'zorazhao@dgcrane.com'); ?>"><?php echo (isset($expertEmail) && ($expertEmail !== '')?$expertEmail:'zorazhao@dgcrane.com'); ?></a></p>
                </div>
            </div>
        </div>
        <!-- EXPERT CONTACT END -->
...
<!-- old -->
    ...
    <script>
        let detailQuoteSubmitting = false;

        function showDetailQuoteMessage(title, message, type) {
        ...
        function clearDetailQuoteForm() {
            $('#detail-inquiry-form .home-collect-name, #detail-inquiry-form .home-collect-email, #detail-inquiry-form .home-collect-phone, #detail-inquiry-form .home-collect-message').val('');
        }

        function getDetailFormattedPhoneNumber($input) {
        ...
        function submitDetailInquiry() {
            let name = $('#detail-inquiry-form .home-collect-name').val();
            let phone = getDetailFormattedPhoneNumber($('#detail-inquiry-form .home-collect-phone'));
            let email = $('#detail-inquiry-form .home-collect-email').val();
            let content = $('#detail-inquiry-form .home-collect-message').val();
            let productId = '<?php echo $product['id']; ?>';

            if (!name || !email || !content) {
                showDetailQuoteMessage('Incomplete Information', 'Please fill in your Name, Email and Message.', 'error');
                return;
            }

            if (detailQuoteSubmitting) {
                return;
            }

            let $button = $('#detail-inquiry-form .home-collect-submit');
            let data = {
                name: name,
                phone: phone,
                email: email,
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.code == 1) {
                        ...
                        showDetailQuoteMessage('Submit Failed', res.msg || 'Please try again later.', 'error');
                    }
                },
                error: function () {
                    showDetailQuoteMessage('Submit Failed', 'Network error. Please try again later.', 'error');
                },
                complete: function () {
                    detailQuoteSubmitting = false;
                    $button.removeClass('is-loading').text('SEND MESSAGE');
                }
            })
        }
    </script>
